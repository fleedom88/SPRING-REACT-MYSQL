...
const imageInputRef = useRef<HTMLInputElement | null>(null); //image의 경로
...
const onImageChangeHandelr = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  imageInputRef.current.value = ''; // 변경되었는지 탐지 하기 위해서
...
const onImageCloseButtonClickHandler = (deleteindex: number) => {
  ...
  const newImageUrls = imageUrls.filter((url, index) => index !== deleteindex );//delete인덱스랑 다른 것들만 남음
  setImageUrls(newImageUrls);
      ...
      <div className='board-update-container'>
        <div className='board-update-box'>
          ...
          <div className='board-update-content-box'>
            <textarea ref={contentRef} className='board-update-content-textarea' placeholder='본문을 작성해주세요.'  onChange={onContentChangeHandler} />
            <div className='icon-button' onClick={onImageUploadButtonClickHandler} >
              <div className='icon image-box-light-icon'></div>
            </div>
            <input ref={imageInputRef} type='file' accept='image/*' style={ { display: 'none' } } onChange={onImageChangeHandelr} />
          </div>
