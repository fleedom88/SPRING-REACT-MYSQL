...
const SignUpCard = () =>{
  ...
  const signUpResponse = (reponseBody: SignUpResponseDto | ResponseDto | null) => {
    if(!reponseBody){
      alert('네트워크 이상입니다.');
      return;
    }
  ...
  const onNextButtonClickHandler = () =>{
    ...
    console.log("이메일"+email.trim().length);
    if(!isEmailPattern){
      setEmailError(true);
      setEmailErrorMessage('이메일 주소 포멧이 맞지 않습니다.');
    }
    const isCheckedPassword = password.trim().length >= 8;
    console.log("비밀번호"+password.trim().length);
  ...
  const onSignUpButtonClickHandler = () =>{
    ...
    const hadNickname = nickname.trim().length > 0;
    if (!hadNickname){
      setNicknameError(true);
    ...
    if(!hadNickname || !isTelNumberPattern|| !agreedPersonal) return;

    const requestBody: SignUpRequestDto = {
      email, password, nickname, telNumber, address, addressDetail, agreedPersonal
    };

    SignUpRequest(requestBody).then(signUpResponse); //signUpRequest API 호출후 signUpResponse메서드 실행
  }
  ...
  const onNinknameKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) =>{
    if(event.key !== 'Enter') return;
    ...
    <div className='auth-card'>
      ...
      <div className='auth-card-top'>
        ...
        {page === 1 && (
          ...
          <InputBox ref={passwordCheckRef} type={passwordCheckType} label='비밀번호 확인*' placeholder='비밀번호를 다시 입력해주세요.' value={passwordCheck} onChange={onPasswordCheckChangeHandler} error={isPasswordError} message={passwordCheckErrorMessage} icon={passwordCheckButtonIcon} onButtonClick={onPasswordCheckButtonClickHandler} onKeyDown={onPasswordCheckKeyDownHandler} /> 
          </>
        )}
        {page === 2 && (
          <>
          <InputBox ref={nicknameRef} label='닉네임*' type='text' placeholder='닉네임을 입력해주세요.' value={nickname} onChange={onNicknameChangeHandler} error={isNicknameError} message={nicknameErrorMessage} onKeyDown={onNinknameKeyDownHandler}/>
