...
import { GetSignInUserResponseDto, GetUserResponseDto, PatchNicknameResponesDto, PatchProfileImageResponseDto } from "./response/user";
...
export const patchNicknameRequest = async (requestBody: PatchNicknameRequestDto, accessToken: string) => {
    ...
    .then(response => {
        const responseBody: PatchNicknameResponesDto = response.data;
        return responseBody;
    })
    .catch(error => {
        if(!error.response) return null;
